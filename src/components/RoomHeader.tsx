    ...
    <div className="flex items-center justify-between gap-2 sm:gap-4 w-full">
      ...
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <button onClick={() => setShowUsers(!showUsers)} className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs text-white/40 hover:text-white/60 transition" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      {/* Right: share */}
      <button onClick={copyLink} className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 sm:py-1.5 rounded-xl text-[10px] sm:text-xs font-medium transition-all duration-200 flex-shrink-0 ${copied ? 'bg-[var(--accent)] text-black' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'}`} style={{ fontFamily: 'var(--font-body)' }}>
        {copied ? (
      ...
      {/* Users dropdown */}
      {showUsers && (
        <div className="absolute top-full left-0 right-0 mt-2 px-3 sm:px-4 pb-3 z-50 animate-fade-in">
